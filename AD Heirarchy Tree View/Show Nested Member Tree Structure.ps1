...
.DESCRIPTION
    ...
    The output of the script will simply flag any circular group nesting. This script assumes
...
$PSBoundParameters | Select-Object -ExpandProperty Keys | ForEach-Object{
    if(((Get-Variable -Name $_ -ValueOnly -ErrorAction SilentlyContinue).GetType()).Name -eq 'string'){
        Set-Variable -Name $_ -Value ((Get-Variable -Name $_ -ValueOnly) -replace "['`"]",'')
        }
    }
...
$i = 1
while($AllMembers.Queried -match $false){
...
while(@($AllMembers | ?{$_.Written -eq $false}).count -ne 0){

    # Was the last thing written the last child for the current parent?
    if($CurrentParent -ne '/' -and @(($AllMembers | ?{$_.Parent -eq $currentparent -and $_.Written -match 'false'})).Count -eq 0){
        # Change parent and indent
        # Using [0] here to make sure we don't get more than one instance of root returned
        $currentParent = ($AllMembers | ?{$_.id -eq $CurrentParent}).Parent[0]
    ...
    $AllMembers | Sort-Object Type -Descending | Where-Object {$_.parent -eq $currentParent -and $_.Written -match 'false'} | %{
        ...
        if($MakePipeable -eq $true){
            ...
            if($color -ne $null){
                Write-Host ($CurrentIndent + $Line) -ForegroundColor $Color
                }
            else{
        ...
        if(($AllMembers | ?{$_.Parent -eq $t.id}) -ne $null){
